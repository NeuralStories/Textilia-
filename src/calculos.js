...
export function calc(h, c) {
  const a = +h.an, al = +h.al;
  const nH = +(h.nc !== undefined ? h.nc : c.nH);
  const fr = +(h.fr !== undefined ? h.fr : c.fr);
  const pT = +(h.pT !== undefined ? h.pT : c.pT);
  const pC = +(h.pC !== undefined ? h.pC : c.pC);
  const pI = +(h.pi !== undefined ? h.pi : c.pI);
  const mg = +(h.mg !== undefined ? h.mg : c.mg);
  const bj = +(h.bj !== undefined ? h.bj : c.bj);
  const ci = +(h.ci !== undefined ? h.ci : c.ci);
  const dA = +(h.dA !== undefined ? h.dA : c.dA);

  const mh = R((a / nH) * fr, 3), mt = R(mh * nH, 3);
  const ct = R(mt * pT, 2), cc = R(mt * pC, 2);
  const ph = R(ct + cc + pI, 2), be = R(ph * (mg / 100), 2), th = R(ph + be, 2);
  const acCon = R(mh + ci, 3), alCon = R(al - dA, 3);
  const aEst = R(Math.ceil(mh / 0.25) * 0.25, 2), alCu = R(Math.ceil(al / 0.03) * 0.03, 2);

  return {
    ...h, mh, mt, ct, cc, ph, be, th, acCon, alCon,
    nc: nH, sm: R(mh * nH, 3), aEst, alCu,
    rf: R(a - 0.01, 3), sop: R(a * 2, 2),
    pT, pC, pi: pI, fr, bj, mg
  };
}
